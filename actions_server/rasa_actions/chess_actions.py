...
class ActionNewChessGame(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain_dict: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        board = chess.Board()
        dispatcher.utter_message(response="utter_board", board=board.unicode())
        return [
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain_dict: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        try:
            ...
            if ai_move in board.legal_moves:
                board.push(ai_move)
                dispatcher.utter_message(response="utter_ai_move", move=str(ai_move), board=board.unicode())
                return [
                    SlotSet("chess_board", board.fen()),
                    FollowupAction("action_check_game_over")
                ]
            else:
                dispatcher.utter_message(response="utter_ai_illegal_move", move=str(ai_move))
                return [FollowupAction("action_new_chess_game")]
        except Exception:
...
class ActionCheckGameOver(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict) -> List[Dict[Text, Any]]:
        ...
        if board.is_game_over():
            ...
            dispatcher.utter_message(response="utter_board", board=board.unicode())
            dispatcher.utter_message(response="utter_new_game_prompt")
